...
function reg_rate = face_rec_eig(X_train, Y_train, X_test, Y_test, k)
    ...
    title("Recognition Rate vs. k for ORL using EIG", 'FontSize', 14, 'FontWeight', 'bold'); 
    grid on;
    xticks(1:length(k));
    xticklabels(k); 
    
    % Save the figure
    exportgraphics(gcf, 'q4-eig-ORL.png', 'Resolution', 300); 
end

%% Function to calculate Recognition Rate using svd method
function reg_rate = face_rec_svd(X_train, Y_train, X_test, Y_test, k)
    ...
    xlabel("k (index)", 'FontSize', 14, 'FontWeight', 'bold'); 
    ylabel("Recognition Rate", 'FontSize', 14, 'FontWeight', 'bold');
    title("Recognition Rate vs. k for ORL using SVD", 'FontSize', 14, 'FontWeight', 'bold');
    grid on;
    xticks(1:length(k)); 
    xticklabels(k);

    % Save the figure
    exportgraphics(gcf, 'q4-svd-ORL.png', 'Resolution', 300); 
end
...
reg_rate_eig = face_rec_eig(X_train, Y_train, X_test, Y_test, k);
reg_rate_svd = face_rec_svd(X_train, Y_train, X_test, Y_test, k);
